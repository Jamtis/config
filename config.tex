...
\declarecommand{\endproof}{~\hfill\originalendproof}


%% Todos
\declarecommand{\marginnote}[1]{}
\ifllncspaper{
    \ifluatex\else
        \declarecommand{\marginpar}{\marginnote}
    \fi
}{
...
%% lists
\ifbeamer{}{
    \declarecommand\labelitemi{$\vcenter{\hbox{\tiny$\bullet$}}$}
...
\declarecommand{\cref}[1]{\ifdisplay\textup{\originalcref{#1}}\else\originalcref{#1}\fi}


%% strikeout in math mode
\let\originalsout\sout
\declarecommand{\sout}[1]{\ifmmode\text{\originalsout{\ensuremath{#1}}}\else\originalsout{#1}\fi}